...
type FavoriteKind = "featured" | "shared" | "private" | "package";
...
function parseFavoriteId(value: string): {
  storageId: string;
  sourceId: string;
  kind: FavoriteKind;
  legacy: boolean;
  ...
  const kind = value.slice(0, separatorIndex) as FavoriteKind;
  const sourceId = value.slice(separatorIndex + 1);

  if (
    !sourceId ||
    !["featured", "shared", "private", "package"].includes(kind)
  ) {
    return null;
  }
...
function favoriteLabel(kind: FavoriteKind) {
  if (kind === "shared") return "حساب مشترك";
  if (kind === "private") return "حساب خاص";
  if (kind === "package") return "بكج ألعاب";
  return "لعبة مميزة";
}
...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    ...
    async function loadFavorites() {
      try {
        ...
        const productIds = Array.from(
          new Set(
            parsedFavorites
              .filter((item) => item.kind !== "package" && isUuid(item.sourceId))
        ...
        const packagesMap = new Map(
          ((packagesResult.data ?? []) as PackageRow[]).map((pkg) => [
            pkg.id,
            pkg,
          ])
        );

        const loadedGames = parsedFavorites
          .map((favorite): FavoriteGame | null => {
            if (favorite.kind === "package") {
              const pkg = packagesMap.get(favorite.sourceId);
            ...
            const product = productsMap.get(favorite.sourceId);
            ...
            return {
              ...
              oldPrice: toNumber(product.old_price ?? product.price),
        ...
        setFavoriteIds(ids);
        setFavoriteGames(loadedGames);
      } catch (error) {
        console.error("تعذر تحميل المفضلة:", error);
    ...
    window.addEventListener("zeta-favorites-updated", handleFavoritesUpdated);
  ...
  function saveFavorites(ids: string[]) {
    ...
    setFavoriteGames((current) =>
      current.filter((game) => ids.includes(game.id))
    );
    localStorage.setItem("zeta_favorites", JSON.stringify(ids));
    window.dispatchEvent(
      new CustomEvent("zeta-favorites-updated", { detail: ids })
    );
  }
  ...
  function clearFavorites() {
    if (!window.confirm("هل أنت متأكد من حذف جميع الألعاب من المفضلة؟")) {
      return;
    }
      ...
      {favoriteGames.length === 0 ? (
        <section className="relative z-10 mx-auto flex min-h-[calc(100vh-180px)] max-w-5xl items-center justify-center px-4 py-10">
          <div className="w-full max-w-xl rounded-[34px] border border-rose-400/15 bg-gradient-to-br from-[#1a1320] via-[#100d18] to-[#0c0a11] px-5 py-11 text-center shadow-2xl">
            <div className="mx-auto flex h-28 w-28 animate-[heartFloat_2400ms_ease-in-out_infinite] items-center justify-center rounded-[34px] border border-white/10 bg-gradient-to-br from-rose-600/25 to-violet-600/15 text-6xl">
              ♡
            </div>
            ...
            <p className="mx-auto mt-3 max-w-sm text-sm leading-7 text-gray-400">
              أضف الألعاب التي تعجبك إلى المفضلة لتجدها هنا بسهولة.
            </p>
            <Link
              href="/#new-games"
              className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-rose-600 to-violet-600 px-7 py-4 text-sm font-black sm:w-auto"
            >
              استكشف الألعاب 🎮
            </Link>
          </div>
          ...
          <div className="mb-7 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-xs text-gray-500">
                لديك {favoriteGames.length} لعبة في المفضلة
              </p>
            </div>
            <button
              type="button"
              onClick={clearFavorites}
              className="rounded-xl border border-red-400/15 bg-red-500/10 px-3 py-2 text-xs font-bold text-red-300 active:scale-95"
            >
              حذف الكل
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {favoriteGames.map((game) => {
              ...
              return (
                <article
                  key={game.id}
                  className={`group overflow-hidden rounded-[24px] border border-white/[0.08] bg-[#111019] shadow-xl transition-all duration-300 ${
                    removingId === game.id
                ...
                >
                  <div className="relative aspect-[4/5] overflow-hidden">
                    ...
                    {discount > 0 && (
                      <span className="absolute right-2 top-2 z-20 rounded-lg bg-red-500 px-2 py-1 text-[9px] font-black">
                        -{discount}%
                      </span>
                    )}
                    ...
                    <button
                      ...
                      className="absolute left-2 top-2 z-20 flex h-9 w-9 items-center justify-center rounded-full border border-rose-400/20 bg-rose-500/15 text-lg text-rose-300 active:scale-90"
                    >
                      ♥
                    </button>
                  </div>
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-3 flex items-end justify-between gap-2">
                      ...
                      <button
                        ...
                        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${accent} text-lg font-black active:scale-90`}
                      >
                        +
                      </button>
                    </div>
