...
export default function SharedGamesPage() {
    ...
    async function loadGames() {
      ...
      try {
        const { data, error } = await supabase
          ...
          .eq("is_active", true)
          .or("display_kind.eq.shared,is_shared.eq.true")
        ...
        const loadedGames = ((data ?? []) as ProductRow[]).map((product) => {
          ...
          return {
            id: product.id,
            name: product.name,
            platform: product.platform || "حساب PC مشترك",
            category:
  ...
  function toggleFavorite(game: Game) {
    const favoriteId = "shared-" + game.id;
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500">ألعاب PC المشتركة</p>
            </div>
          </div>
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-4 pt-5">
        <div className="relative overflow-hidden rounded-[32px] border border-violet-400/15 bg-[radial-gradient(circle_at_75%_18%,rgba(139,92,246,0.28),transparent_35%),linear-gradient(135deg,#1a1325,#0f0c17_70%)] px-5 py-10 sm:px-8 sm:py-14">
          ...
          <p className="mt-4 max-w-md text-sm leading-7 text-gray-400">
            استمتع بألعاب الكمبيوتر بأسعار أقل مع تعليمات استلام واضحة.
          </p>
        </div>
